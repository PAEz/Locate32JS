...
gui.progress = 0;
gui.progressLast = 0;
var guiKeys = Object.keys(gui);
guiKeys.forEach(function(key){
	gui[key].enabledLast=!gui[key].enabled;
})


function onFrame() {
	// if(locate.scan.scanning){
	if (gui.progress !== gui.progressLast) {
		$.progress.style.width = gui.progress + '%';
		gui.progressLast = gui.progress;
		// }
	} else {
		var update = false;
	...
	for (var i = 0, iEnd = guiKeys.length; i < iEnd; i += 1) {
		var key = guiKeys[i];
		if (gui[key].enabled != gui[key].enabledLast) {
			if (gui[key].enabled) {
				gui[key].el.removeAttribute('disabled');
			} else {
				gui[key].el.setAttribute('disabled', '');
			}
		}
		gui[key].enabledLast = gui[key].enabled;
	}
...
$.fileInput.addEventListener('change', function(e) {
	...
	if (files.length) {
		...
		gui.generate.enabled=false;
		gui.save.enabled=false;
...
gui.save.el.addEventListener('click', function(e) {
...
gui.generate.el.addEventListener('click', function(e) {
	var totals = 0,
		totalsCount = 0;
	var volumes = [];
					...
					switch (item.event) {
						...
						case 'end':
							...
							if (next) scanIt(next);
								...
								gui.save.enabled=true;
	gui.load.enabled=true;
	gui.generate.enabled=true;
              ...
              */
					...
					totalsCount += 1;
					gui.progress = (totalsCount / totals) * 100;
